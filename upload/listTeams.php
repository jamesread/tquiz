...
$sql = 'SELECT t.id, t.title, t.userCount, t.registered, count(p.id) AS level FROM teams t LEFT JOIN team_progress p ON p.team = t.id WHERE t.quiz = :activeQuiz GROUP BY t.id ORDER BY level DESC, registered DESC';
$stmt = $db->prepare($sql);
