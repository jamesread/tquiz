...
if ($team) {
    ...
    if (empty($listLevels) || count($completedLevels) == count($listLevels)) {
        $tpl->display('allLevelsComplete.tpl');

        $completedness = 100;
    } else {
        $completedness = intval((count($completedLevels) / count($listLevels)) * 100);
    }

    $tpl->assign('completedness', $completedness);
    $tpl->assign('completedLevels', $completedLevels);
    $tpl->assign('listLevels', $listLevels);
} else {
    $tpl->assign('listLevels', []);
}
